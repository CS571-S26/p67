                    ...
                    return <Row key={"Row " + index1} xs={5}>
                        {ar.map((state, index2) => {
                            return <Col key={"Col " + index2}>
                                <Button variant={buttonStates[index1][index2] ? "primary" : "danger"} onClick={() => setButtons(buttonStates.map((arr, i1) => arr.map((item, i2) => Math.abs(i1 - index1) + Math.abs(i2 - index2) <= 1 ? !item : item)))}>
